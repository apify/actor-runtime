...
- Podman (3.4 or newer) works the same way: serve its API socket and mount it in place of the Docker one, e.g. `-v /run/podman/podman.sock:/var/run/docker.sock` (rootless: `$XDG_RUNTIME_DIR/podman/podman.sock`). Create the `data` directory first. See README.md's "Running with Podman" section.
