...
smoke_tests() {
	...
	"$cli" version
	if [ "$cli" = podman ]; then
		"$cli" info --format 'Arch={{.Host.Arch}} OS={{.Host.Distribution.Distribution}} {{.Host.Distribution.Version}}'
	else
	...
	# A plain string, expanded unquoted below: macOS's bash 3.2 treats an empty array expansion as an
